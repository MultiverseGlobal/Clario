...
You are adopting this repository’s design operating system as the default standard for every product interface you create, modify, review, or propose. Treat the files referenced below as working rules, not as a mood board. Before writing code, plan the product experience, inspect the existing codebase and assets, and explain the design decisions you are adopting. For a full product, follow `@full-platform-development-phases.md` and use `@full-platform-build.md` as the default end-to-end workflow.
...
- `@expo-react-native-mobile.md`
- `@full-platform-development-phases.md`
- `@platform-architecture.md`
- `@database-api-backend.md`
...
Use `@taste-engine.md` on every design task to infer the brief, calibrate variance/motion/density, judge taste, enforce restraint, and run the anti-slop pre-flight. Use `@immersive-components-and-animation.md` when the product needs expressive interaction, stateful animation, spatial continuity, data storytelling, or a memorable signature. Use `@platform-architecture.md`, `@database-api-backend.md`, and `@reliability-security-performance.md` whenever the task crosses client, server, database, API, authentication, deployment, or performance boundaries. Use `@expo-react-native-mobile.md` whenever the project uses Expo, React Native, Expo Router, NativeWind, or React Native Reanimated. Use `@user-taste-and-wonder-profile.md` to understand the project’s preference for high taste, restraint, strategic challenge, wonder, and cross-device craft. Immersion must remain useful, accessible, interruptible, performant, and understandable without effects.
