...
app.post('/api/webhooks/atlas', async (req: Request, res: Response) => {
  try {
    ...
    // Save to William's Action Log / Queue
    await saveActionLog({
      id: `act_${Date.now()}`,
    ...
    res.status(200).json({ success: true, message: 'Prospect queued in William.' });
  } catch (error) {
...
async function fetchMetaphorBrief(): Promise<MetaphorBrief | null> {
  ...
  try {
    ...
    return null; // Metaphor offline — William still works fine
  }
}
...
async function generateWilliamResponse(
  ...
  if (hasKeys) {
    try {
      const systemPrompt = buildWilliamSystemPrompt(portrait, worldContext, insights, timeOfDay, metaphorBrief);
...
app.post('/api/reasoner', async (req: Request, res: Response) => {
  try {
    ...
    // Note: frontend already saves the user message, so we only save william's response here
...
app.get('/api/briefings/morning', async (_req: Request, res: Response) => {
  try {
    ...
    const briefingPrompt = `You are William. Generate a concise, personalised morning briefing for ${portrait.name}.
...
Write 2–3 sentences. Be direct, human, and specific. No bullet points. Reference at least one concrete item from their schedule or priorities.`;
    ...
    const response = await BrainGateway.execute({
      systemPrompt: 'You are William, a calm and insightful executive companion.',
...
app.post('/api/metaphor/webhook', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, message: `William received ${action}` });
  } catch (error) {
